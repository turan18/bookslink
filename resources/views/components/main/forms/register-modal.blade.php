<div id="register-modal" class="fixed top-1/4 left-1/2 transform -translate-x-1/2 -translate-y-1/4 opacity-0 pointer-events-none transition duration-500 ease-in-out rounded-lg z-10">
    <div class="content text-black">
        <form method="POST" action="/register" id="register-form" class="p-10 bg-white rounded flex justify-center items-center flex-col shadow-md">
            ...
            <p class="mb-5 text-3xl uppercase text-gray-600">Register Now</p>

            @error('username','register')
            <span class="text-red-500 mb-2">{{$message}}</span>
            @enderror
            <input type="text" id="username" name="username" value="{{old('username')}}" class="mb-5 p-3 w-80 focus:border-purple-700 rounded border-2 outline-none" autocomplete="off" placeholder="Username" required>


             @error('email','register')
                <span class="text-red-500 mb-2">{{$message}}</span>
             @enderror
            <input type="email" id="email" name="email" value="{{old('email')}}" class="mb-5 p-3 w-80 focus:border-purple-700 rounded border-2 outline-none" autocomplete="off" placeholder="Email" required>
            ...
            <x-main.buttons.auth>Register</x-main.buttons.auth>
                <button type="button" id="register-close" class="bg-indigo-600 hover:bg-indigo-500 text-white font-bold p-2 rounded w-20"><span>Close</span></button>
        </form>
    </div>
</div>
